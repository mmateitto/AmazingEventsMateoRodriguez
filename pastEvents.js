...
let arrayPasado = data.events.filter(element => data.currentDate > element.date);
...
pintarTarjeta(padreTarjetas, arrayPasado);
...
padreCheckbox.addEventListener('change', (event) => {
    ...
    let filtradosBox = arrayPasado.filter(objeto => {
        for (let i = 0; i < array.length; i++) {
    ...
    if (filtradosBox.length === 0) {
        pintarTarjeta(divPadre, arrayPasado);
    } else {
...
buscar.addEventListener('input', (event) => {
    let filtradosSearch = arrayPasado.filter(e =>
